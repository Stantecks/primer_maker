...
parser.add_argument("--custom_str", help="additional string to append to primer names", default="")
parser.add_argument('--gg5', metavar="ATGCx4", help="5 prime golden gate overhang. GG function assumes PCR is for scar-less insert")
parser.add_argument('--gg3', metavar="ATGCx4", help="3 prime golden gate 'underhang' (type out 5' to 3')")
...
def tmrev(dna): 
    seqdnas = Seq(dna)
    dna = str(seqdnas.reverse_complement())
    revdna_list = []
    revtms_list = []
    count = 14
    while count < 61:
...
def label_maker(gene_name, anneal, fwd=True, custom_str=args.custom_str):
    if fwd == True:
...
if args.well96:
    idt_order = idt_order[["Well Position", "Name", "Sequence"]]
    idt_order.to_excel(args.out+".xls", index=False, sheet_name=args.name96)
else:
    idt_order.to_csv(args.out+".csv", index=False)
